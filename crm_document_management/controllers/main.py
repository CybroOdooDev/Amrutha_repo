...
class Sign(http.Controller):
    ...
    def get_document_qweb_context(self, sign_request_id, token, **post):
        ...
        # Fetch the current request item based on the token
        ...
        # Fetch the sign request and crm.lead.document
        sign_request = http.request.env['sign.request'].sudo().browse(
            sign_request_id)
        crm_lead_document = http.request.env[
            'crm.lead.document'].sudo().search([
            ('sign_request_id', '=', sign_request.id)
        ], limit=1)

        # Initialize lead_id as None
        lead_id = None

        if crm_lead_document:
            # List the Many2one fields to check
            lead_fields = [
                'residential_sale_id', 'residential_purchase_id',
                'commercial_sale_id',
                'commercial_purchase_id', 'auction_sale_id',
                'auction_purchase_id'
            ]

            # Iterate over the fields and get the first non-empty lead id
            for field in lead_fields:
                if getattr(crm_lead_document, field):
                    lead_id = getattr(crm_lead_document, field)
                    break  # Stop once the first valid lead_id is found

        # Now `lead_id` holds the ID of the related crm.lead record (or None if not found)
        crm_lead = lead_id
        print(crm_lead,"ghhhhhhhhhh")
        # crm_lead = http.request.env['crm.lead'].sudo().browse(1213)
        # if not crm_lead:
        #     return request.not_found()  # If the crm.lead with id=4 doesn't exist

        # Set the context language from crm.lead partner_id's language (or default if none)
        if crm_lead:
            if crm_lead.partner_id.lang:
                http.request.env.context = dict(http.request.env.context,
                                                lang=crm_lead.partner_id.lang)

        sign_item_types = http.request.env[
            'sign.item.type'].sudo().search_read([])
        if not sign_item_types:
            raise UserError(
                _("Unable to sign the document due to missing required data. Please contact an administrator."))

        # Define allowed item types for signing
        item_type_signature = request.env.ref('sign.sign_item_type_signature',
                                              raise_if_not_found=False)
        item_type_initial = request.env.ref('sign.sign_item_type_initial',
                                            raise_if_not_found=False)
        item_type_text = request.env.ref('sign.sign_item_type_text',
                                         raise_if_not_found=False)
        ...
        for item_type in sign_item_types:
            item_type['edit_while_signing_allowed'] = item_type[
                                                          'id'] in edit_while_signing_allowed_type_ids
        ...
        if current_request_item:
            for item_type in sign_item_types:
                ...
                if item_type['item_type'] in ['signature', 'initial']:
                    ...
                    item_type[
                        'auto_value'] = 'data:image/png;base64,%s' % user_signature.decode() if user_signature else False
                    item_type[
                        'frame_value'] = 'data:image/png;base64,%s' % user_signature_frame.decode() if user_signature_frame else False

                if current_request_item.state == 'sent':
                    # Update location if necessary (same logic as before)
        ...
        return {
            ...
            'state_to_sign_request_items_map': dict(
                tools.groupby(sign_request.request_item_ids,
                              lambda sri: sri.state)),
            'token': token,
            'nbComments': len(sign_request.message_ids.filtered(
                lambda m: m.message_type == 'comment')),
            'isPDF': (sign_request.template_id.attachment_id.mimetype.find(
                'pdf') > -1),
            'webimage': re.match('image.*(gif|jpe|jpg|png|webp)',
                                 sign_request.template_id.attachment_id.mimetype),
            ...
            'readonly': not (
                        current_request_item and current_request_item.state == 'sent' and sign_request.state in [
                    'sent', 'shared']),
            'sign_item_types': sign_item_types,
            'sign_item_select_options': sign_request.template_id.sign_item_ids.mapped(
                'option_ids'),
            'portal': post.get('portal'),
            'company_id': (
                        sign_request.communication_company_id or sign_request.create_uid.company_id).id,
            'today_formatted_date': format_date(http.request.env,
                                                fields.Date.today(),
